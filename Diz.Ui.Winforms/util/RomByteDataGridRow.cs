...
public class RomByteDataGridRow : RomByteRowBase, IDataGridRow
{
    ...
    private void SetStyleForIndirectAddress(string colPropName, DataGridViewCellStyle style)
    {
        ...
        {
            nameof(Offset) => 
                SnesData.IsMatchingIntermediateAddress(selectedRomByteRow.ParentIndex, ByteEntry.ParentIndex),
            nameof(IA) => 
                SnesData.IsMatchingIntermediateAddress(ByteEntry.ParentIndex, selectedRomByteRow.ParentIndex),
            _ => false
        };
